...
function Sports() {
    ...
    const getSports = () => {
        const data = axios.get('https://api.nytimes.com/svc/topstories/v2/sports.json?api-key=YXk5msAL9qVSUhGbaClNKaXYtEvJq7Ae')
        .then(res=> setSports(res.data.results.splice(3,20)))
    }
    
    const getSports2 = () => {
        const data = axios.get('https://api.nytimes.com/svc/topstories/v2/sports.json?api-key=YXk5msAL9qVSUhGbaClNKaXYtEvJq7Ae')
        .then(res=> setSports2(res.data.results.splice(0,3)))
    }
