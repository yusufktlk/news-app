...
function Movies() {
    ...
    const getMovies = () => {
        const data = axios.get('https://api.nytimes.com/svc/topstories/v2/movies.json?api-key=YXk5msAL9qVSUhGbaClNKaXYtEvJq7Ae')
        .then(res=> setMovies(res.data.results.splice(3,20)))
    }
    
    const getMovies2 = () => {
        const data = axios.get('https://api.nytimes.com/svc/topstories/v2/movies.json?api-key=YXk5msAL9qVSUhGbaClNKaXYtEvJq7Ae')
        .then(res=> setMovies2(res.data.results.splice(0,3)))
    }
